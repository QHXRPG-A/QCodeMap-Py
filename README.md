...
```
                 ...
                 │            既有手段的长短版图                 │
                 │                                             │
   codemap CLI ──┤  结构视角：import 图 / 枢纽 / 影响面          │
   （结构层）     │  ✗ 无调用链 ✗ ast-grep 30s 超时              │
                 │                                             │
   jedi + .pyi ──┤  语义视角：跨文件引用 / 类型推导              │
   （语义层）     │  ✗ 大库静默丢边 ✗ 12~17s/查询                │
                 │  ✗ .pyi 必须与源码同目录，无法分离            │
                 └──────────────────┬──────────────────────────┘
                                    │ 取两者之长
                 ┌──────────────────▼──────────────────────────┐
                 │                  QCodeMap                    │
                 │                                            │
                 │   结构能力（codemap 平价）    语义能力（超 jedi） │
                 │   deps/importers/hubs/tree   callers/callees  │
                 │   blast-radius 影响面         VERIFIED 语义验证 │
                 │   ✓ 无超时（纯查表）           ✓ 亚秒（两阶段） │
                 │                              ✓ RPC/事件约定边 │
                 │                                            │
                 │        框架动态语义的「桩知识」                 │
                 │   .pyi 桩文件 ──数据化──▶ SQLite 事实表        │
                 │   （同目录约束消失，与源码物理分离）            │
                 │        ▲                                   │
                 │        │ 第五类钩子：习语规则插拔              │
                 │   custom/（config / facts / seeds）           │
                 │   组件注入 · 声明式属性 · 全局注入 · RPC 分发   │
                 └────────────────────────────────────────────┘
                                    │
                 ┌──────────────────▼──────────────────────────┐
                 │              消费面（AI / MCP）               │
                 │   CLI 13 命令 · stdio MCP server（13 工具）   │
                 │   懒刷新 · 覆盖率契约 · context 冷启动档案     │
                 └────────────────────────────────────────────┘
```

一句话：**取 codemap 的结构能力与 jedi 的语义能力，把 .pyi 桩降格为
SQLite 数据行**——桩与源码分离、查询从现扫变查表、习语经 custom 插拔。
